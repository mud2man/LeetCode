 ...
 * 2. Keep update the GCD by split phase divide nums[i] and gcd 
 */

import java.util.*; // Stack

public class Solution {
    private int splitPhaseDivide(int num0, int num1){
        int small = Math.min(num0, num1);
        int big = Math.max(num0, num1);
        if(small == big || small == 0){
            return big;
        }else{
            return splitPhaseDivide(small, big % small);
        }
    }
    ...
    public boolean isGoodArray(int[] nums) {
        ...
        for(int i = 1; i < nums.length; ++i){
            gcd = splitPhaseDivide(gcd, nums[i]);
        }
        return (gcd == 1);
    }
