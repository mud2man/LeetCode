...
public class Solution{
    private int rabinKarpSearch(String S, int len){
        ...
        for(int end = len - 1; end < S.length(); ++end){
            ...
            hash = (((hash - (long)(startChar - 'a') * maxCoefficient) % base + base) * 26 + (long)(endChar - 'a')) % base; //add base to prevent underflow
    ...
    public String longestDupSubstring(String S) {
        ...
        while(l <= r){
            int mid = (l + r) / 2;
            int startIdx = rabinKarpSearch(S, mid);
            if(startIdx != -1){
                l = mid + 1;
                longestDuplicate = (longestDuplicate.length() < mid)? S.substring(startIdx, startIdx + mid): longestDuplicate;
            }else{
                r = mid - 1;
            }
        }
    ...
    public static void main(String[] args){
        ...
        System.out.println("longest duplicate substring:" + sol.longestDupSubstring(S));
    }
}
